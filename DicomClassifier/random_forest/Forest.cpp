...
Forest::Forest() :
    verbose_out(nullptr), num_trees(DEFAULT_NUM_TREE), mtry(0), min_node_size(0), num_independent_variables(0), seed(0), num_samples(
...
// #nocov start
void Forest::initCpp(const std::string& dependent_variable_name, MemoryMode memory_mode, const std::string& input_file, uint mtry,
                     const std::string& output_prefix, uint num_trees, std::ostream* verbose_out, uint seed, uint num_threads,
                     const std::string& load_forest_filename, ImportanceMode importance_mode, uint min_node_size,
                     const std::string& split_select_weights_file, const std::vector<std::string>& always_split_variable_names,
                     const std::string& status_variable_name, bool sample_with_replacement,
                     const std::vector<std::string>& unordered_variable_names, bool memory_saving_splitting, SplitRule splitrule,
                     const std::string& case_weights_file, bool predict_all, double sample_fraction, double alpha, double minprop, bool holdout,
...
void Forest::init(std::unique_ptr<Data> input_data, uint mtry, const std::string& output_prefix,
...
// #nocov start
void Forest::loadFromFile(const std::string& filename) {
    if (verbose_out)
...
void Forest::loadDependentVariableNamesFromFile(const std::string& filename) {
    ...
    for (size_t i = 0; i < num_dependent_variables; ++i) {
        ...
        infile.read(temp, length * sizeof(char));
        temp[length] = '\0';
...
#ifdef OLD_WIN_R_BUILD
// #nocov start
void Forest::showProgress(const std::string& operation, clock_t start_time, clock_t& lap_time) {
...
// #nocov end
#else
void Forest::showProgress(const std::string& operation, size_t max_progress) {
